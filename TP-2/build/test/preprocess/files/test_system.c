...
void test_invalid_sensor_value_workflow(void) {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        int sensor_value = rand() % 200 - 100;

        if (sensor_value <= 0 || sensor_value >= 100) {

            notify_threshold_exceeded(sensor_value);

            do {if ((sensor_value <= 0 || sensor_value >= 100)) {} else {UnityFail( ((" Expected TRUE Was FALSE")), (UNITY_UINT)((UNITY_UINT)(49)));}} while(0);
    ...
    for (int i = 0; i < sizeof(boundary_values) / sizeof(boundary_values[0]); i++) {
        ...
        if (is_valid_value(sensor_value)) {
           ...
           ((void *)0)
            ...
            do {if ((sensor_value <= 0 || sensor_value >= 100)) {} else {UnityFail( ((" Expected TRUE Was FALSE")), (UNITY_UINT)((UNITY_UINT)(68)));}} while(0);
