...
void setUp(void) {
    // Initialiser le générateur de nombres aléatoires avec une seed fixe pour des résultats reproductibles
    srand(42);
}
...
void test_invalid_sensor_value_workflow(void) {
    for (int i = 0; i < 20; i++) {  // Vous pouvez augmenter le nombre d'itérations pour plus de couverture
        ...
        if (sensor_value < 0 || sensor_value > 100) {
            ...
            TEST_ASSERT_FALSE(is_valid_value(sensor_value)); // Vérifiez que la condition d'invalidité est respectée
        }
    }
}
